...
class AccesoDatos {
   ...
   // Constructor privado  Patron singleton
    ...
    public function getByOrder($ordenar,$primero,$cuantos){
        $tuser = [];
    ...
    public function repetido($email){
        $consulta = $this->dbh->prepare("select * from Clientes where email=?");
    ...
    public function repetidoMod($email){
        $consulta = $this->dbh->prepare("select * from Clientes where email=?");
        $consulta->bind_param("s",$email);
        $consulta->execute();
        $result = $consulta->get_result();
        if ( $result ){
            $clie = $result->fetch_object('Cliente');
        }
        return $clie;
    }

    public function comprobarInicio($nombre,$passw){
        $user = false;
        
        $stmt_usuario   = $this->dbh->prepare("select * from user where nombre =? and password=?");
        if ( $stmt_usuario == false) die ($this->dbh->error);

        // Enlazo $login con el primer ? 
        $stmt_usuario->bind_param("ss",$nombre,$passw);
        $stmt_usuario->execute();
        $result = $stmt_usuario->get_result();
        if ( $result ){
    ...
    public function repetidoInicio($nombre){
        $consulta = $this->dbh->prepare("select * from user where nombre=?");
